...
def pack_input(sex, age, loyalty, distance, p_class, travel_type, dep_delay, arr_delay, wifi, fun,
    ...
    """ translates input values to pass to model """
...
with tab1:
    st.write('Exploratory data analysis: исследуем наши данные, предварительно очищенные и обработанные :sparkles: :sparkles:')

    st.write('**Возраст пассажиров**')
    age = Image.open('data/Age.png')  # мб вынести наверх
    st.image(age)
    st.divider()

    st.write('**Категориальные признаки:** пол, лояльность, тип поездки и удовлетворённость')
    cat = Image.open('data/Categorical.png')  # мб вынести наверх
    st.image(cat)
    st.divider()

    st.write('**Расстояние полёта**')
    dist = Image.open('data/Distance.png')  # мб вынести наверх
    st.image(dist)
    st.divider()

    st.write('**Задержка вылета и прилёта**')
    delays = Image.open('data/Delays.png')  # мб вынести наверх
    st.image(delays)
    st.divider()

    st.write('**Постполётные опросы** (14 вопросов по шкале от 0 до 5)')
    scores = Image.open('data/Scores.png')  # мб вынести наверх
    st.image(scores)
    st.divider()

    # TODO добавить описания по 1-2 строки к каждому

with tab2:
    st.write('Введите данные вашего пассажира:')
    st.button('Сгенерировать случайно')
    ...
    # TODO кнопка подставить случайные значения во все поля
    # TODO украсить код, мб выделить функции
    # TODO деплой
    # TODO ридми
    # TODO переписать анализ вклада факторов
